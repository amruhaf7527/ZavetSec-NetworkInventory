...
<#
.SYNOPSIS
    ZavetSec-NetworkInventory - Network subnet scanner and asset inventory.
.DESCRIPTION
...
.EXAMPLE
    ...
    .\ZavetSec-NetworkInventory.ps1 -Subnets '10.0.0.0/24','10.0.1.0/24' -Threads 100
    .\ZavetSec-NetworkInventory.ps1 -Subnets '192.168.1.0/24' -SkipPing -ScanShares
.NOTES
    Version : 1.0
...
$portServices = @{
    ...
    8009='AJP'; 10000='Webmin'
                # Huawei OUIs resolved via maclookup.app API
                # Proxmox / QEMU
                'BC2411'='Proxmox'
                # ZTE
...
foreach ($ip in $liveArray) {
    ...
    [void]$ps.AddScript({
        ...
        # OUI vendor lookup - local DB first, then maclookup.app API fallback
        if ($macAddress) {
            $oui6 = $macAddress.Substring(0,8).Replace(':','').ToUpper()
            # 1. Try local DB (fast, offline)
            if ($ouiDb -and $ouiDb.ContainsKey($oui6)) {
                $vendor = $ouiDb[$oui6]
            } else {
                # 2. Try maclookup.app API (accurate, requires internet)
                try {
                    $apiUrl  = "https://api.maclookup.app/v2/macs/$macAddress/company/name"
                    $wc      = [System.Net.WebClient]::new()
                    $wc.Headers.Add('User-Agent', 'ZavetSec-NetInventory/1.0')
                    $result = $wc.DownloadString($apiUrl).Trim()
                    $wc.Dispose()
                    if ($result -and $result -ne '' -and $result -notmatch 'Not Found|ERROR') {
                        # Normalize: remove common suffixes, trim to 30 chars
                        $result = $result -replace '\s*(CO\.|LTD\.?|INC\.?|CORP\.?|LLC\.?|TECHNOLOGIES|COMMUNICATION|ELECTRONIC[S]?|COMPUTER[S]?|NETWORK[S]?|SYSTEMS?|INTERNATIONAL|ENTERPRISE[S]?)\s*$','' -replace '\s+CO\.,\s*LTD\.?$','' 
                        $result = $result.Trim().Trim(',').Trim()
                        if ($result.Length -gt 30) { $result = $result.Substring(0,30).Trim() }
                        $vendor = if ($result) { $result } else { 'Unknown' }
                    } else {
                        $vendor = 'Unknown'
                    }
                } catch { $vendor = 'Unknown' }
            }
        }
...
$hostRows = foreach ($h in $allResults) {
    ...
    foreach ($p in $h.OpenPorts) {
        ...
        $pc = '#ff6b00'
        # Badge color by service category
        $bg = ''
        if ($sn -ne '') {
            $bgColor = if ($pi -eq 22 -or $pi -eq 23)                                          { '#0a2e1a;color:#30d158' }  # SSH/Telnet - green
                       elseif ($pi -eq 3389)                                                    { '#1a0a2e;color:#a78bfa' }  # RDP - purple
                       elseif ($pi -eq 5985 -or $pi -eq 5986)                                  { '#0a1a2e;color:#00d4ff' }  # WinRM - cyan
                       elseif ($pi -eq 445 -or $pi -eq 139 -or $pi -eq 137 -or $pi -eq 135)   { '#2e1a0a;color:#ff6b00' }  # SMB/RPC - orange
                       elseif ($pi -eq 80 -or $pi -eq 8080 -or $pi -eq 8008 -or $pi -eq 8000) { '#0a1520;color:#4a9eff' }  # HTTP - blue
                       elseif ($pi -eq 443 -or $pi -eq 8443 -or $pi -eq 9443)                 { '#0a1a28;color:#00d4ff' }  # HTTPS - cyan
                       elseif ($pi -eq 21 -or $pi -eq 20 -or $pi -eq 990 -or $pi -eq 989)     { '#2e200a;color:#ffd60a' }  # FTP - yellow
                       elseif ($pi -eq 25 -or $pi -eq 465 -or $pi -eq 587)                    { '#201a0a;color:#e8a000' }  # SMTP - amber
                       elseif ($pi -eq 53 -or $pi -eq 88 -or $pi -eq 389 -or $pi -eq 636)     { '#1a1a2e;color:#8888ff' }  # DNS/LDAP - indigo
                       elseif ($pi -eq 1433 -or $pi -eq 1521 -or $pi -eq 3306 -or $pi -eq 5432 -or $pi -eq 27017 -or $pi -eq 6379 -or $pi -eq 5984 -or $pi -eq 9200 -or $pi -eq 9042) { '#2e0a0a;color:#ff2d55' }  # DB - red
                       elseif ($pi -eq 2375 -or $pi -eq 2376 -or $pi -eq 2377)                { '#1a0a20;color:#c084fc' }  # Docker - violet
                       elseif ($pi -eq 3000 -or $pi -eq 5601 -or $pi -eq 9090 -or $pi -eq 9100 -or $pi -eq 4040) { '#0a2020;color:#34d399' }  # Monitoring - emerald
                       elseif ($pi -eq 161 -or $pi -eq 162)                                   { '#1e1a0a;color:#f59e0b' }  # SNMP - amber
                       else                                                                     { '#181828;color:#6e6e80' }  # default - gray
            $bg = "<span style='display:inline-block;padding:1px 6px;border-radius:3px;font-size:9px;font-weight:700;background:#" + ($bgColor -split ';color:#')[0].TrimStart('#') + ";color:#" + ($bgColor -split ';color:#')[1] + "'>" + $sn + "</span>"
        }
    ...
    $dataRisks  = ($riskItems -join ' | ') -replace '"',''
    $dataPorts  = @($h.OpenPorts | ForEach-Object {
        $pi2 = [int]"$_"
        $sn2 = if ($portServices.ContainsKey($pi2)) { $portServices[$pi2] } else { '' }
        if ($sn2) { "$pi2/$sn2" } else { "$pi2" }
    }) -join ' | '
    $dataMac    = $macStr -replace '"',''
    $dataVendor = $vendorStr -replace '"',''
    [void]$row.Append("<tr data-risk=`"$($h.RiskLevel)`" data-ip=`"$($h.IP)`" data-hn=`"$hn`" data-os=`"$os`" data-dom=`"$dom`" data-smb=`"$($h.SmbSigning)`" data-ports=`"$dataPorts`" data-risks=`"$dataRisks`" data-mac=`"$dataMac`" data-vendor=`"$dataVendor`">")
...
function exportCSV() {
    ...
    var lines = [cols.join(',')];
    function q(v) { var s = String(v || ''); s = s.split(dq).join(dq+dq); return dq + s + dq; }
    for (var i = 0; i < rows.length; i++) {
        if (rows[i].style.display === 'none') continue;
        var d = rows[i].dataset;
        if (!d || !d.ip) continue;
        lines.push([q(d.risk),q(d.ip),q(d.hn),q(d.os),q(d.dom),q(d.smb),q(d.ports),q(d.risks),q(d.mac),q(d.vendor)].join(','));
    }
    var csv = lines.join(String.fromCharCode(13,10));
    var fn = 'NetworkInventory_' + new Date().toISOString().slice(0,10) + '.csv';
...
<footer>
  Generated: $($global:StartTime.ToString('yyyy-MM-dd HH:mm:ss')) | ZavetSec-NetworkInventory v1.0 | Subnets: $subnetStr | CONFIDENTIAL - SOC/IS USE ONLY &nbsp;&nbsp;|&nbsp;&nbsp; <a href='https://github.com/zavetsec' style='color:#3a3a55;text-decoration:none;font-size:10px' onmouseover="this.style.color='#f5c518'" onmouseout="this.style.color='#3a3a55'">github.com/zavetsec</a>
</footer>
</body>
</html>
"@
